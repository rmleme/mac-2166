...
int leia_seq_genetica(char dna[])
{
  FILE *arqentrada;
  char nome[TAM_NOME];
  int n = 0;
  ...
  else
  {
    ...
    fclose(arqentrada);
    if (n == 0)
      printf("Nenhuma sequencia valida foi encontrada no arquivo %s\n",nome);
  }
  return n;
}
...
void leia_proteina(int *n, char proteina[])
{
  FILE *arqentrada;
  char nome[TAM_NOME];
  int qtas_prot = 0;
...
void testa_sequencia(char sequencia[], int qtos_dna, char seq_proteica[],
                     int qtas_prot, char nome_sequencia[])
{
  int indice1,
      indice2,
      indice3;
  ...
  {                                                  // toda a sequencia de DNA
    ...
    {
      printf("Encontrada uma ocorrencia comecando no caractere %d %s\n",
             indice1 + 1,nome_sequencia);
      printf("A sequencia e' a seguinte: ");
      for (indice3 = indice1; indice3 <= 3 * qtas_prot + indice1; indice3++)
        printf("%c",sequencia[indice3]);
      printf("\n                           ");
      for (indice3 = 0; indice3 <= qtas_prot - 1; indice3++)
        printf("%c  ",seq_proteica[indice3]);
      printf("\n");
    }
  }
}
...
// Funcao: main
// Descricao: abre os arquivos de DNA e de proteina e, caso existam, testa as
//            sequencias geneticas em busca da proteina.
